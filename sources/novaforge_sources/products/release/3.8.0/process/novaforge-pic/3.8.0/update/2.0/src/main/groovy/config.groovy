...
// Configure sonar plugin for Jenkins
def sonarPublisherTmp = processTmpPath + "/resources/hudson.plugins.sonar.SonarPublisher.xml"
...
beaverEngine.replaceElement(sonarPublisherTmp, "@BASE_URL@", baseUrl)
...
beaverEngine.replaceElement(sonarPublisherTmp, "@SONAR_DEFAULT_ALIAS@", defaultAlias)
...
beaverEngine.replaceElement(sonarPublisherTmp, "@SONAR_TOOL_ALIAS@", toolAlias)

def mariadbHost = beaverEngine.getResource("mariadb","host")
def mariadbPort = beaverEngine.getResource("mariadb","port")
beaverEngine.replaceElement(sonarPublisherTmp, "@MYSQL_HOST@", mariadbHost)
beaverEngine.replaceElement(sonarPublisherTmp, "@MYSQL_PORT@", mariadbPort)
def jenkinsDatas = beaverEngine.getResource("jenkins","datas")

def sonarPublisherXml = jenkinsDatas + "/hudson.plugins.sonar.SonarPublisher.xml"
beaverEngine.copyToFile(sonarPublisherTmp, sonarPublisherXml)
beaverEngine.setOwner(true,localGroup,localUser,sonarPublisherXml)
