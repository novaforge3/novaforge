...
# vim:ft=perl:et:sw=4
# $Id: List.pm 12704 2016-02-26 08:06:10Z sikeda $
...
sub replace_topic_title {
    ...
    #my $file = Sympa::Tools::File::get_filename('etc',{},'topics.conf',$robot);
	my $file = Sympa::search_fullpath($robot, 'topics.conf');
	
    my $tmp_file = "$Conf::Conf{'etc'}/tmp.txt";

    unless (open (FILE, "<", $file)) {
	$log->syslog('err',"Unable to open config file $file");
    }

    unless (open (TMP_FILE, ">", $tmp_file)) {
	$log->syslog('err',"Unable to open temporary file $tmp_file");
    }
...
##Â 3. title --- the title of the topic
##Â 4. visibility --- the visibility of the topic
sub create_topic {
    my $robot = shift;
    my $name = shift;
    my $title = shift;
    ...
    #my $conf_file = Sympa::Tools::File::get_filename('etc',{},'topics.conf',$robot);
	my $conf_file = Sympa::search_fullpath($robot, 'topics.conf');

    unless (open (FILE, ">>", $conf_file)) {
	$log->syslog('err',"Unable to open config file $conf_file");
	return undef;
    }
...
## This subroutine returns true if the topic exists
## Parameters are :
## 1. robot
## 1. name the name of the topic (id)
sub exist_topic {    
    ...
    unless (%all_topics = Sympa::Robot::load_topics($robot)) {
	$log->syslog('err', 'exist_topic(): load_topics_failed');
	return undef;
    }
...
sub exist_subtopic {    
	...
	$log->syslog('err', 'exist_subtopic(): problem on the subtopic structure name with name=%s', $name);
    	return undef;
    }

    unless (%all_topics = Sympa::Robot::load_topics($robot)) {
	$log->syslog('err', 'exist_subtopic(): load_topics_failed');
	return undef;
    }
...
sub check_admin_to_delete {
    ...
    die sprintf 'Invalid parameter: %s', $email
        unless ref $email;    
	die sprintf 'Invalid parameter: %s', $listname
        unless ref $listname;   
	...
	unless (
            ...
            $log->syslog('err',
                'Unable to retrieve user admin for list %s', $listname);
            $sth = pop @sth_stack;
            return undef;
        }
	...
	while ($data = $sth->fetchrow_hashref) 
	{
	 my $admin=$data->{'user_admin'}; 
	 $log->syslog('info',
            '%s is admin of  : %s',$admin,$listname);
	  if ($email eq  $admin) 
	  { 
	    $is_admin=1; 
	  }	
...
sub get_list_description {
    my $self = shift;
    my $result;
      if (-r $self->{'dir'}.'/info') {
	 my $file_path = $self->{'dir'}.'/info';
	 unless (open FILE, "<", $file_path) {
	     $log->syslog('err', 'Cannot open file %s', "$file_path");
	     return undef;
	 }
	 while (<FILE>) {
...
sub get_list_template {
    my $self = shift;
    my $result;
      if (-r $self->{'dir'}.'/template') {
	 my $file_path = $self->{'dir'}.'/template';
	 unless (open FILE, "<", $file_path) {
	     $log->syslog('err', 'Cannot open file %s', "$file_path");
	     return undef;
	 }
...
sub update_owner_email {
    my $self = shift;
    my $old_email = shift;
    my $new_email = shift;
    my $data;
    my $file_path = $self->{'dir'}.'/config';

## Lock file
    my $lock = Sympa::LockedFile->new($file_path,2,'>');
    unless (defined $lock) {
		$log->syslog('err','Could not create new lock');
		return undef;
    }
...
# read file
    unless (open FILE, "<:encoding(UTF-8)", $file_path) {
	$log->syslog('err', 'Cannot open file %s for reading', "$file_path");
	return undef;
    }
...
# copy config file
   my $index = $self->get_configfile_last_index();
   my $new_file_path = $self->{'dir'}.'/config.'."$index";
   $log->syslog('info', 'new file is %s', "$new_file_path");
   unless (open FILE, ">:encoding(UTF-8)", $new_file_path) {
	$log->syslog('err', 'Cannot open file %s for writing', "$new_file_path");
	return undef;
    }
...
# Substitute old email with new email
    $data =~ s/$old_email/$new_email/g;

# rewrite file
    unless (open FILE, ">:encoding(UTF-8)", $file_path) {
	$log->syslog('err', 'Cannot open file %s for writing', "$file_path");
	return undef;
    }
    print FILE $data;
    close FILE;
...
# update admin_table
    unless ($self->update_owner_email_db($old_email, $new_email)) {
        $log->syslog('err', 'List:update_owner_email(%s): Failed to update %s %s', $self->{'name'}, $old_email, $new_email);
	return undef;
    }
...
sub update_owner_email_db {
    my($self, $old_email, $new_email) = @_;
	$log->syslog('info',
            'List::update_owner_email_db(%s,%s,%s)', $self->{'name'},$old_email, $new_email);
	...
	unless (
            ...
            and $sth = $sdm->do_prepared_query(
                q{UPDATE admin_table
				  SET user_admin=?
                  WHERE (user_admin=? AND list_admin=? AND role_admin=?)},
                $new_email,$old_email,$self->{'name'},'owner'
            )
            ) {
            $log->syslog('err',
                'Unable to update owner email for user admin %s for the list admin %s', $old_email,$self->{'name'});
            $sth = pop @sth_stack;
            return undef;
        }   
    return 1;
...
sub get_configfile_last_index {
   ...
   unless (opendir(DIR, $dir)) {
      do_log('err',"Unable to open $dir");
      return undef;
   }
   
   foreach (grep(/^config.\d/,readdir(DIR))){
      @tab = split("config.", $_);
      push @result, $tab[1];
   }
   closedir DIR;
