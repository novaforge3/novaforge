...
# vim:ft=perl:et:sw=4
# $Id: Admin.pm 12670 2016-02-05 15:35:02Z david.verdin $
...
sub create_list_old {
    ...
    ## template file creation. Use UTF-8 for command line orgigin only.
    if($origin eq "command_line") {
    unless (open TEMPLATE, '>:utf8', "$list_dir/template") {
        &do_log('err','Impossible to create %s/template : %s',$list_dir,$!);
    }
    }
    else {
    unless (open TEMPLATE, '>:bytes', "$list_dir/template") {
        &do_log('err','Impossible to create %s/template : %s',$list_dir,$!);
    }
    }
    if (defined $list_tpl) {
    Encode::from_to($list_tpl, 'utf8', $Conf::Conf{'filesystem_encoding'});
    print TEMPLATE $list_tpl;
    }
    close TEMPLATE;
