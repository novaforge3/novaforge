...
def distributionLevel = beaverEngine.getResource("level")
...
if ("0".equals(distributionLevel)) {
  def newFeature = "<feature version=\"" + dataVersion + "\">novaforge-distribution-full</feature>"
  def oldFeature = beaverEngine.getValueFromRegex(deployFile, "<feature version=\"(.*)\">novaforge-distribution-full</feature>")
  ...
  def newFeature = "<feature version=\"" + dataVersion + "\">novaforge-distribution-client</feature>"
  def oldFeature = beaverEngine.getValueFromRegex(deployFile, "<feature version=\"(.*)\">novaforge-distribution-client</feature>")
  if (StringUtils.isNotBlank(oldFeature)) {
    beaverEngine.replaceElement(deployFile, oldFeature, newFeature)
  } else {
    def lineAdded = beaverEngine.addLineToFile(deployFile,pluginsToken,newFeature)
