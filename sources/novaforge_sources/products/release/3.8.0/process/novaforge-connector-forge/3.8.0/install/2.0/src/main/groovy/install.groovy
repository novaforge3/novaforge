...
def list = adminForbiddenLogins.split(',').collect{it as String}
if (list.contains(adminLogin))
{
     throw new BeaverException(String.format("This configured admin login (%s) is part of forbidden logins. Please choose another one.",adminLogin))
}
...
// Update initialisation start
def dataVersion = beaverEngine.getDataVersion()
def deployFile = beaverEngine.getResource("novaforge-runtime", "deployFile")
def initFalse = "<bundle start='false'>mvn:org.novaforge.forge.modules/novaforge-initialization/" + dataVersion + "</bundle>"
def initTrue = "<bundle start='true'>mvn:org.novaforge.forge.modules/novaforge-initialization/" + dataVersion + "</bundle>"
beaverEngine.replaceElement(deployFile, initFalse, initTrue)
