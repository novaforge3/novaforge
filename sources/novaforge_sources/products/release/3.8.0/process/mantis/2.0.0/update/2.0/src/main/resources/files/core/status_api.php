...
#############################
/**
 * Create a language enumeration specified by the array of languages
 * @param array $p_instance_file
 * @param string $p_label_status
 * @param array $p_languages
 * @param int $p_line - null by default
 */
function create_languages_status(&$p_instance_file, &$p_label_status, &$p_languages, &$p_line = null) {

	$language_status_enum = "\t".'$s_'.$p_label_status."_enum_language = '";
	foreach ( $p_languages as $language ) {
		$language_status_enum .= $language.",";
	}
	$language_status_enum = substr($language_status_enum,0,-1);
	$language_status_enum .="';\n";
	

	if($p_line == null) {
	    $line = get_line_in_file($p_instance_file, "#enum_language#");
		if( $line != false ) {
				$line += 2;
				$p_instance_file = insert_in_file($p_instance_file, $language_status_enum, $line);
		}
	}
	else {
		$p_instance_file[$p_line] = $language_status_enum;	
	}
}

/**
 * Add a language to a status
 * @param array $p_instance_file
 * @param string $p_label_status
 * @param string $p_language
 * @param int $p_line
 */
function add_language_status(&$p_instance_file, &$p_label_status, &$p_language, &$p_line)	{
	$p_instance_file[$p_line] = substr(trim($p_instance_file[$p_line]), 0, -2).",".$p_language."';\n";
}

/**
 * Delete a language enumeration specified by the label' status
 * @param array $p_instance_file
 * @param string $p_label_status
 */
function delete_languages_status(&$p_instance_file, &$p_label_status) {
	$line = get_line_in_file($p_instance_file, "s_".$p_label_status."_enum_language");
	if ( $line != false ) {
		$p_instance_file = remove_in_file($p_instance_file, $line);
	}	
}
 ...
 */
function update_name_status(&$p_instance_file, &$p_label_status, &$p_status_multi_language, &$p_languages_to_add) {
	$languages = array_diff(config_get( 'language_choices_arr' ), session_get('error_language'));
	$t_enum_status = MantisEnum::getAssocArrayIndexedByValues( config_get( 'status_enum_string' ) );
	foreach ( $languages as $language) {
		if( in_array($language, $p_languages_to_add) ) {
			$t_new_name_status = addslashes($p_status_multi_language[$language][NAME]);
		}
		else {
			$t_new_name_status = addslashes($p_status_multi_language[DEFAULT_LANGUAGE][NAME]);
		}
		...
		foreach ( $t_name_status_array as $t_id_status => $t_name_status ) {
			if ( $t_actual_id_status == $t_id_status ) {
				...
				$t_new_names_status .= $t_id_status.':'.addslashes($t_name_status).',';
			}
		}
...
function status_create(&$p_label_status, &$p_color_status, &$p_status_multi_language) {
    ...
    create_languages_status($instance_file, $p_label_status, $languages);
	...
	# --Notification Email-- 
	if ( !is_blank($p_status_multi_language[DEFAULT_LANGUAGE][EMAIL]) ) {
...
function status_update(&$p_label_status, &$p_color_status, &$p_status_multi_language) {
	...
	# --Languages--
	$new_status_multi_language = array();
	$t_languages_to_add = array();
	$t_languages_to_delete = array();
	...
	#languages to add
	foreach ( $p_status_multi_language as $language => $status_language ) {
       		array_push($t_languages_to_add, $language);
	}
	#languages to delete
	foreach ( $language_array as $existing_language ) {
       if ( !in_array($existing_language, $t_languages_to_add) ) {
       		array_push($t_languages_to_delete, $existing_language);
       }
	}

	# --Name--
	update_name_status($instance_file, $p_label_status, $p_status_multi_language, $t_languages_to_add);
	
	# --Manage Title Button & Email--
	# --for deleting languages
	foreach ( $t_languages_to_delete as $t_language_to_delete ) {
		delete_field_status($instance_file, $p_label_status, TITLE, $t_language_to_delete);
		delete_field_status($instance_file, $p_label_status, BUTTON, $t_language_to_delete);	
		delete_field_status($instance_file, $p_label_status, EMAIL, $t_language_to_delete);
	}
	...
	foreach ( $p_status_multi_language as $language => $status_language ) {
		...
		if ( in_array($language, $language_array) ) {
			...
			$new_status_multi_language[$language][TITLE] = $status_language[TITLE];
			$new_status_multi_language[$language][BUTTON] = $status_language[BUTTON];
			$new_status_multi_language[$language][EMAIL] = $status_language[EMAIL];
		}
	}
	# --for creating languages
	if ( count($new_status_multi_language) != 0 ) {
		$t_id_status = MantisEnum::getValue( config_get('status_enum_string'), $p_label_status);		
		foreach ( $new_status_multi_language as $language => $status_language ) {		
			create_field_status($instance_file, $p_label_status, TITLE, $status_language[TITLE], $language);
			create_field_status($instance_file, $p_label_status, BUTTON, $status_language[BUTTON], $language);
			create_field_status($instance_file, $p_label_status, EMAIL, $status_language[EMAIL], $language);
		}
	...
	# --Languages--
	$t_line = get_line_in_file($instance_file, $p_label_status.'_enum_language');
	create_languages_status($instance_file, $p_label_status, $t_languages_to_add, $t_line);
...
function status_delete( &$p_label_status) {
	...
	# --Enum languages
	delete_languages_status($instance_file, $p_label_status);
...
/**
 * Check if datas (title, button and email) exist for a specified language in the instance conf file. If not, create it
 * @param string $p_language
 * @return boolean $ret
 */
function check_datas_initial_status(&$p_language) {
	
	if (session_get( 'project_id') != null)
	{
		$ret = false;
		...
		# --Label
		$line = get_line_in_file($instance_file, "g_status_enum_string");
		if ( $line == false ) {
			initialize_label_status($instance_file);
			$ret = true;
		}
				
		# --Color
		$line = get_line_in_file($instance_file, "g_status_color");
		if ( $line == false) {
			initialize_color_status($instance_file);
			$ret = true;
		}
			
		# --Name
		$line = get_line_in_file($instance_file, "status_enum_string_".$p_language);
		if ( $line == false) {
			initialize_name_status($instance_file);
			$ret = true;
		}  
		
		foreach ( $t_enum_status as $t_to_status_id => $t_to_status_label ) {
			# --Enum language
			$line = get_line_in_file($instance_file, $t_to_status_label."_enum_language");
			# not existing
			if ( $line == false ) {
				initialize_languages_status($instance_file, $t_to_status_label, $p_language);
				$ret = true;
			}
			# existing but not containing the $p_language
			elseif ( !ereg($p_language,$instance_file[$line] ) ) {
				add_language_status($instance_file, $t_to_status_label, $p_language, $line);				
			}  
	       	# --Title
			$line = get_line_in_file($instance_file, $t_to_status_label."_bug_title_".$p_language);
			if ( $line == false ) {
				initialize_field_status($instance_file, $t_to_status_label, TITLE, $p_language);
				$ret = true;
			}   	
			# --Button
			$line = get_line_in_file($instance_file, $t_to_status_label."_bug_button_".$p_language);
			if ( $line == false ) {
				initialize_field_status($instance_file, $t_to_status_label, BUTTON, $p_language);
				$ret = true;
			}	
			# --Email
			$line = get_line_in_file($instance_file, "email_notification_title_for_status_bug_".$t_to_status_label.'_'.$p_language);
			if ( $line == false ) {
				initialize_field_status($instance_file, $t_to_status_label, EMAIL, $p_language);
				$ret = true;
			}
		}
		if ( $ret ) {
...
/**
 * Create in the specified file the name for specified status for all languages
 * @param array $p_instance_file
 */
function initialize_name_status(&$p_instance_file) {
	$t_arr = config_get( 'language_choices_arr' );
	$t_all_language = array_diff($t_arr, session_get('error_language'));
	$line = get_line_in_file($p_instance_file, "#name#");
	if( $line != false ) {
		$line += 2;
	
		foreach ( $t_all_language as $t_language ) {
				$t_status_enum_string = MantisEnum::getAssocArrayIndexedByValues( lang_get( 'status_enum_string', $t_language ) );
				$status_name = "\t".'$s_status_enum_string_'.$t_language." = '";
				foreach ( $t_status_enum_string as $t_status_id => $t_status_label ) {
					if ( !($t_status_enum_string[$t_status_id] == null or $t_status_enum_string[$t_status_id] == "")) {
					   $status_name .= $t_status_id.':'.addslashes($t_status_enum_string[$t_status_id]).',';			
					}
				}
				$status_name = substr($status_name, 0, -1);
				$status_name .= "';\n";
				$p_instance_file = insert_in_file($p_instance_file, $status_name, $line);
		}
	}
}

/**
 * Initialize specified field for a specified status
 * @param array $p_instance_file
 * @param string $p_label_status
 * @param string $p_field, the type of field to create
 * @param string $p_language - null by default
 */
function initialize_field_status(&$p_instance_file, &$p_label_status, $p_field, &$p_language = null) {
	switch ( $p_field ) {
		case TITLE:
			$t_sentence = "\t".'$s_'.$p_label_status."_bug_title_$p_language = '".addslashes(lang_get($p_label_status."_bug_title"))."';\n";
			break;
		case BUTTON:
			$t_sentence = "\t".'$s_'.$p_label_status."_bug_button_$p_language = '".addslashes(lang_get($p_label_status."_bug_button"))."';\n";
			break;
		case EMAIL:
			$t_sentence = "\t".'$s_email_notification_title_for_status_bug_'.$p_label_status."_$p_language = '".addslashes(lang_get('email_notification_title_for_status_bug_'.$p_label_status))."';\n";
			break;
	}
...
/**
 * Initialize in the specified file the enum language for specified status in specified language
 * @param array $p_instance_file
 * @param string $p_status_label
 * @param string $p_language
 */
function initialize_languages_status(&$p_instance_file, &$p_status_label, &$p_language) {

	$status_enum_language = "\t".'$s_'.$p_status_label."_enum_language = '".$p_language."';\n";
	
	$line = get_line_in_file($p_instance_file, "#enum_language#");
	if( $line != false ) {
		$line += 2;
		$p_instance_file =  insert_in_file($p_instance_file, $status_enum_language, $line);
	}	
}

/**
 * Initialize in the specified file the color
 * @param array $p_instance_file
 */
function initialize_color_status(&$p_instance_file) {
	
	#get all existing status
	$t_enum_status = MantisEnum::getAssocArrayIndexedByValues( config_get( 'status_enum_string' ) );
	
	#create the new variable
	$new_colors_status = "\t".'$g_status_colors = array('."\n";
	foreach ( $t_enum_status as $t_to_status_id => $t_to_status_label ) {		
		$new_colors_status .= "\t'".$t_to_status_label."'\t=> '".get_status_color($t_to_status_id)."',\n";
	}
	$new_colors_status = substr(trim($new_colors_status),0,-1);
	$new_colors_status .= "\n);\n";
	
	#get the line to insert the data
	$line = get_line_in_file($p_instance_file, "#status_colors#");
	if( $line != false ) {
		$line += 2;
		$p_instance_file = insert_in_file($p_instance_file, $new_colors_status, $line);
	}
}

/**
 * Initialize in the specified file the label status
 * @param array $p_instance_file
 */
function initialize_label_status(&$p_instance_file) {	
	#get all existing status
	$t_enum_status = MantisEnum::getAssocArrayIndexedByValues( config_get( 'status_enum_string' ) );
	...
	#get the line to insert the data
	$line = get_line_in_file($p_instance_file, "#status_enum_string#");
	if( $line != false ) {
		$line += 2;	
		$p_instance_file = insert_in_file($p_instance_file, $new_status_enum, $line);	
	}
}
