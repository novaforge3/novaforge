...
def mantisMigrationStatusLogFile = logs + "/mantis_MigrationStatus.log"
Path mantisMigrationStatusLogFilePath = Paths.get(mantisMigrationStatusLogFile)
if (Files.exists(mantisMigrationStatusLogFilePath))
{
	beaverEngine.delete(mantisMigrationStatusLogFile)
}
beaverEngine.copyToFile(processTmpPath + "/resources/log/mantis.log", mantisMigrationStatusLogFile)
...
// Status Migration
String[] curlStatusArgs = new String[7] 
curlStatusArgs[0] = "--noproxy"
curlStatusArgs[1] = "-k" 
curlStatusArgs[2] = "-X" 
curlStatusArgs[3] = "POST" 
curlStatusArgs[4] = "http://" + localhost + "/" + mantisDefaultAlias + "/" + mantisToolAlias + "/admin/migration_etat.php"
curlStatusArgs[5] = "--output" 
curlStatusArgs[6] = mantisMigrationStatusLogFile
...
println "Database Mantis Migration ..."
...
databaseThread.join();
boolean keepOn = databaseRunnable.getKeepOn();

if (keepOn) {
  println "Status Mantis Migration ..."
  beaverEngine.executeCommand(curlCommand, curlStatusArgs)
  
   logFile = new File(mantisMigrationStatusLogFile)
   msgStopEnglish = "Operation successful."
   msgStopFrench = "Opération réussie."
   
   def statusRunnable = new monRunnable()
   statusRunnable.setLogfile(logFile)  
   statusRunnable.setMsgEnglish(msgStopEnglish)
   statusRunnable.setMsgFrench(msgStopFrench)
     
   def statusThread = new Thread (statusRunnable)
   statusThread.start()
   statusThread.join()  
}
