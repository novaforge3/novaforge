...
public class ColumnUserGenerator implements ColumnGenerator
{
  ...
  public Object generateCell(final com.vaadin.ui.Table pSource, final Object pItemId, final Object pColumnId)
  {
    ...
    final UserProfile userProfile = (UserProfile) item.getItemProperty(
        CommitItemProperty.AUTHOR_PROFILE.getPropertyId()).getValue();
    if (userProfile != null)
    {
      ...
      Resource icon;
      if ((userProfile.getImage() != null) && (userProfile.getImage().getFile() != null))
      {
      ...
      returnComponent = button;
    }
    else
    {
      final Label label = new Label("unknown");
      final String userLogin = (String) item.getItemProperty(CommitItemProperty.AUTHOR_LOGIN.getPropertyId())
          .getValue();
      final String userEmail = (String) item.getItemProperty(CommitItemProperty.AUTHOR_EMAIL.getPropertyId())
          .getValue();
      if (!Strings.isNullOrEmpty(userLogin))
      {
        label.setCaption(userLogin);
      }
      else if (!Strings.isNullOrEmpty(userEmail))
      {
        label.setCaption(userEmail);
      }
