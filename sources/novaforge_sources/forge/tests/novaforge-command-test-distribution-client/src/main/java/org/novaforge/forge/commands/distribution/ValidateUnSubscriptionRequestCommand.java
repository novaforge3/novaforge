...
/**
 * This command (launched onto CENTRAL forge) will validate the un-subscription.
 * 
 * @author Marc Blachon
 */
@Command(scope = "distribution", name = "validate-unsubscription",
    description = "validate the received request for unsubscribing")
public class ValidateUnSubscriptionRequestCommand extends OsgiCommandSupport
{
  ...
  protected Object doExecute() throws Exception
  {
    ...
    if (forgeRequests.size() == 1)
    {
      for (final org.novaforge.forge.distribution.register.domain.ForgeRequestDTO forgeRequestDTO : forgeRequests)
      {
        ...
        forgeDistributionService.approveUnsubscription(requestId);
        System.out.println("Un-Subscription request has been validated");
      }
    }
    else if (forgeRequests.size() > 1)
    {
      System.err.println("More than one UNSUBSCRIBE request are pending to be validated");
    }
    else
    {
