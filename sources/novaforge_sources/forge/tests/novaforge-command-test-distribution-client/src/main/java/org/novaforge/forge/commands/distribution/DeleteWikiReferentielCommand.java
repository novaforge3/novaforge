 ...
 * @author Marc Blachon
 */
@Command(scope = "distribution", name = "delete-wiki-page-ref",
    description = "delete a page to the Dokiwiki from the referential project")
public class DeleteWikiReferentielCommand extends OsgiCommandSupport
{
    ...
    try
    {
      ...
      if (!references.isEmpty())
      {
        ...
        ItemReferenceDTO newPageReference = new ItemReferenceDTO(referenceToolProjectId + DOKUWIKI_PAGE_NAME,
            "1--49", "PAGE");
        ItemDTO newPageItem = new ItemDTO(newPageReference, ActionType.DELETE);
        DataDTO newPageData = new DataDTO(newPageItem, newPage);
        pluginDataService.putData(forgeId, pluginInstanceId, newPageData);
        System.out.println("Dokuwiki page deleted: "+DOKUWIKI_PAGE_NAME);
      }
    }
    finally
    {
