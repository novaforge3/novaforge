 ...
 * @author Marc Blachon
 */
@Command(scope = "distribution", name = "fetch-wiki-page-ref",
    description = "Fetch a page to the Dokiwiki from the referential project")
public class FetchWikiReferentielCommand extends OsgiCommandSupport
{
  ...
  protected Object doExecute() throws Exception
  {
    ...
    try
    {
      ...
      {
        final DokuwikiPageDTO newPage = new DokuwikiPageDTO(referenceToolProjectId + DOKUWIKI_PAGE_NAME,
            DOKUWIKI_PAGE_DESCRIPTION, DOKUWIKI_PAGE_CONTENT);
        final ItemReferenceDTO newPageReference = new ItemReferenceDTO(referenceToolProjectId
            + DOKUWIKI_PAGE_NAME, "1--49", "PAGE");
        final ItemDTO newPageItem = new ItemDTO(newPageReference, ActionType.NONE);
        final DataDTO newPageData = new DataDTO(newPageItem, newPage);
        final DataDTO fetchedPage = pluginDataService.getData(forgeId, pluginInstanceId, newPageItem);

        if (newPageData.getDokuwikiPageDTO().getContent()
            .equals(fetchedPage.getDokuwikiPageDTO().getContent()))
        {
          System.out.println("Dokuwiki page: " + fetchedPage.getDokuwikiPageDTO().getName());
        }
        else
        {
