...
public class TestLinkPropagationTest extends ToolsPropagationItBaseTest
{
  private static final Log log = LogFactory.getLog(TestLinkPropagationTest.class);
  private Properties       rolesTestlink;
  private Properties       mappingRolesTestlink;

  @Override
  public void setUp() throws Exception
  {
    super.setUp();
    rolesTestlink = new Properties();
    InputStream fis = this.getClass().getResourceAsStream("/rolesTestlink.properties");
    rolesTestlink.load(fis);

    // mappingRolesTestlink.properties properties file
    mappingRolesTestlink = new Properties();
    fis = this.getClass().getResourceAsStream("/mappingRolesTestlink.properties");
    mappingRolesTestlink.load(fis);
  }
  ...
  public void test01GetRolesMapping() throws Exception
  {
    ...
    for (Iterator<String> iterator = xmlData.getProjects().keySet().iterator(); iterator.hasNext();)
    {
      ...
      String app_name = xmlData.getApplicationName(project, TestConstants.TEST_TYPE);
      assertNotNull("Le nom d'appli non renseigne", app_name);

      Map<String, String> roleCvs = xmlData.getAppMapping(project, TestConstants.TEST_TYPE);
      ...
      for (Iterator<String> iterator1 = roleCvs.keySet().iterator(); iterator1.hasNext();)
      {
        ...
        assertEquals("Le role n'est pas equivalent d'apres le csv / Plugin de Forge", roleCvsTestlink,
  ...
  public void test02LookAtDatabaseTestlink() throws Exception
  {
    ReportTest.writeTestResult(this.getClass().getName(), this.getName(), false);
    log.info("***************************************************************************");
    log.info("******************           Look Role At Database  Testlink      ********************");
    log.info("***************************************************************************");
    for (Iterator<String> iterator = xmlData.getProjects().keySet().iterator(); iterator.hasNext();)
    {
      String project = iterator.next();
      ...
      // check propagation into Testlink Db
      checkPropagationIntoTestLinkDb(project, false, null, null, TestConstants.TEST_TYPE);
    }
    ReportTest.writeTestResult(this.getClass().getName(), this.getName(), true);
  }

  private void checkPropagationIntoTestLinkDb(String project, boolean modifyRoleforPropagation, String userFound,
                                              String roleTarget, String appType) throws Exception
  {
    try
    {
      ...
      Map<String, String> rolesDatabaseTestlink = getRoleDatabaseTestlink(projectName);
      assertNotNull("Aucune role trouve dans le file de conf Testlink", rolesDatabaseTestlink);
      ...
      Map<String, String> roleCvsTestlink = xmlData.getAppMapping(project, appType);
      assertNotNull("Aucune correspondance dans les csv entre users et Mantis", roleCvsTestlink);
      ...
      for (Iterator<String> iterator1 = userAndRoles.keySet().iterator(); iterator1.hasNext(); )
      {
        ...
        String roleDatabase = rolesDatabaseTestlink.get(user);
        assertNotNull("The tool role does not exist for the user " + user, roleDatabase);
        String roleForge = userAndRoles.get(user);
        String roleTestlinkForge = roleCvsTestlink.get(roleForge);

        // TODO: use forge service to get the mapping betwing role in base and role into forge tool
        // mapping
        String resolvedRole = (String) mappingRolesTestlink.get(roleTestlinkForge);

        manageACL.setIfACLError(user, roleDatabase, resolvedRole);
      }
  ...
  private Map<String, String> getRoleDatabaseTestlink(String projet) throws Exception
  {
    Map<String, String> returned = new HashMap<String, String>();
    String              jdbcUrl  = getMySqlJdbcUrl(TestConstants.TEST_TYPE);
    Connection          con      = DriverManager.getConnection(jdbcUrl, TestConstants.USER, TestConstants.PASSWORD);
    String query = "SELECT t.prefix,u.login,r.description "
                       + "FROM users as u, user_testproject_roles as ur,roles as r, testprojects as t"
                       + " WHERE ur.user_id=u.id and ur.role_id=r.id and ur.testproject_id=t.id AND t.prefix LIKE  \""
                       + projet + "%\"";
    ...
    while (resultSet.next())
    {
      ...
      String access_name = resultSet.getString(3);
      log.info("projetname :" + projetname + " username  : " + username + " access_name :" + access_name);
  ...
  public void test03LookProjectAtDataseTestlink() throws Exception
  {
    ReportTest.writeTestResult(this.getClass().getName(), this.getName(), false);
    log.info("***************************************************************************");
    log.info("******************           LookProjectAtDatase  Testlink      ********************");
    log.info("***************************************************************************");
    for (Iterator<String> iterator = xmlData.getProjects().keySet().iterator(); iterator.hasNext();)
    {
      String project = iterator.next();
      ...
      String projetDB = getProjetDatabaseTestlink(projectName);
      assertNotNull("Projet non trouvé  dans la db Testlink", projetDB);
      log.info(projetDB);
    }
    ReportTest.writeTestResult(this.getClass().getName(), this.getName(), true);
  }

  private String getProjetDatabaseTestlink(String projet) throws Exception
  {
    String     returned = null;
    String     jdbcUrl  = getMySqlJdbcUrl(TestConstants.TEST_TYPE);
    Connection con      = DriverManager.getConnection(jdbcUrl, TestConstants.USER, TestConstants.PASSWORD);
    String query = "SELECT t.prefix  " + "FROM testprojects as t " + " WHERE t.prefix LIKE  \"" + projet + "%\"";
    Statement stmt      = con.createStatement();
    ResultSet resultSet = stmt.executeQuery(query);
    while (resultSet.next())
    {
  ...
  public void test04LookAtUserTestlink() throws Exception
  {
    ReportTest.writeTestResult(this.getClass().getName(), this.getName(), false);
    List<String> userTestlink = getUsersDBTestlink();
    assertNotNull("Aucune ser trouve dans le file de conf Testlink", userTestlink);
    ...
    for (Iterator<String> iterator = xmlData.getProjects().keySet().iterator(); iterator.hasNext();)
    {
      ...
      {
        String user = iterator1.next();
        assertTrue("le user n'existe pas dans Testlink", userTestlink.contains(user));
      }
    }
    ReportTest.writeTestResult(this.getClass().getName(), this.getName(), true);
  }

  private List<String> getUsersDBTestlink() throws Exception
  {
  ...
  /*
   * testing propagation into Testlink when changing forge role.
   */
  public void test05TestRoleChangingPropagationIntoTestLink() throws Exception
  {
    ReportTest.writeTestResult(this.getClass().getName(), this.getName(), false);

    log.info("***************************************************************************");
    log.info("******************           Look Role change At Database  Testlink      ********************");
    log.info("***************************************************************************");
    for (Iterator<String> iterator = xmlData.getProjects().keySet().iterator(); iterator.hasNext();)
    {
      String project = iterator.next();
      ...
      if (project.equals(projectTestId) && userIdToChange != null)
      {
        try
        {
          ...
          // check propagation into Testlink Db
          checkPropagationIntoTestLinkDb(project, true, userIdToChange, roleTarget, TestConstants.TEST_TYPE);
        }
        finally
        {
  ...
  /*
   * testing propagation into Testlink when changing roles mappings of Testlink application.
   */
  public void test06TestChangingRoleMappingPropagationIntoTestlink() throws Exception
  {
    ReportTest.writeTestResult(this.getClass().getName(), this.getName(), false);
    log.info("***********************************************************************************");
    log.info("********* Test propagation when changing roles mapping for Testlink *******************");
    log.info("***********************************************************************************");
    ...
    for (Iterator<String> iterator = xmlData.getProjects().keySet().iterator(); iterator.hasNext();)
    {
      ...
      try
      {
        ...
        Map<String, String> newCvsRoleMapping = xmlData.getAppMapping(project, TestConstants.TEST_TYPE2);
        assertNotEquals("Aucun role trouve dans le fichier csv de changement de mapping", true,
            newCvsRoleMapping.isEmpty());
        updateApplication(project, applicationUri, newCvsRoleMapping, TestConstants.TEST_TYPE, app_name);
        ...
        checkPropagationIntoTestLinkDb(project, false, null, null, TestConstants.TEST_TYPE2);
      }
      // always done eveen getting failure
      finally
      {
        // setting back the roles mapping
        Map<String, String> initialCvsRoleMapping = xmlData.getAppMapping(project, TestConstants.TEST_TYPE);
        assertNotEquals("Aucun role trouve dans le fichier csv initial de mapping", true,
            initialCvsRoleMapping.isEmpty());
        updateApplication(project, applicationUri, initialCvsRoleMapping, TestConstants.TEST_TYPE, app_name);
        Thread.currentThread().sleep(TestConstants.WAIT_CHANGE_ROLEMAPPING_PROPAGATION);
      }
  ...
  /*
   * testing propagation into Testlink when changing the role of forge group for a group member
   */
  public void test07TestGroupRoleChangingPropagationIntoTestlink() throws Exception
  {
    ReportTest.writeTestResult(this.getClass().getName(), this.getName(), false);
    log.info("*****************************************************************************");
    ...
    for (Iterator<String> iterator = xmlData.getProjects().keySet().iterator(); iterator.hasNext();)
    {
      ...
      List<String> userTestlink = getUsersDBTestlink();
      assertNotNull("Aucune user trouve dans le file de conf Testlink", userTestlink);
      ...
      if (groupAndRoleschangingCondition.size() == 1)
      {
        ...
        try
        {
          ...
          // check propagation into Testlink Db
          checkGroupRoleChangingPropagation(project, groupId, newRole, TestConstants.TEST_TYPE);
        }
        finally
        {
          // back changing the role for the last found user.
      ...
      String app_type) throws Exception
  {
    try
    {
      ...
      Map<String, String> rolesDatabaseTestlink = getRoleDatabaseTestlink(projectName);
      assertNotNull("Aucune role trouve dans la DB Testlink", rolesDatabaseTestlink);

      Map<String, String> roleCvsTestlink = xmlData.getAppMapping(project, app_type);
      assertNotNull("Aucune correspondance dans les csv entre users et Testlink", roleCvsTestlink);
