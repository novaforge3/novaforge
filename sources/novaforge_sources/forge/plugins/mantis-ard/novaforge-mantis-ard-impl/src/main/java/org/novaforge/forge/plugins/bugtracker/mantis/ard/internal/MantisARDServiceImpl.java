...
import org.novaforge.forge.plugins.bugtracker.mantis.ard.constants.MantisRolePrivilege;
import org.novaforge.forge.plugins.bugtracker.mantis.ard.services.MantisARDConfigurationService;
 ...
 * @author BILET-JC
 */
public class MantisARDServiceImpl extends AbstractPluginService implements PluginService
{
	 ...
	 */
	public MantisARDServiceImpl(final UuidDAO pUuidDAO)
	{
		super(pUuidDAO);
	}
	...
	protected List<PluginView> buildPluginViews()
	{
		...
		MantisARDConfigurationService mantisARDConfiguration = getPluginConfigurationService(MantisARDConfigurationService.class);
		viewList.add(new PluginViewImpl(PluginViewEnum.ADMINISTRATION, mantisARDConfiguration.getAdminAccess()));
		return viewList;
	}
	...
	public PluginServiceMetadata getMetadata() throws PluginServiceException
	{
		...
		if (getServiceUUID() != null)
		{
			pluginServiceMetadata = new PluginMetadataImpl(getServiceUUID(), getDescription(), getType(),
					Category.ARD.getId(), new MantisARDQueues(), getPluginViews(), getVersion());
		}
