...
/**
 * This is an implementation of PluginService for sonar plugin.
 * 
 ...
 */
public class SonarServiceImpl extends AbstractPluginService implements PluginService
{
	 ...
	 */
	public SonarServiceImpl(final UuidDAO pUuidDAO)
	{
		super(pUuidDAO);
	}
  ...
  protected String getIconPath()
  {
	...
	protected List<PluginView> buildPluginViews()
	{
		...
		SonarConfigurationService sonarConfiguration = getPluginConfigurationService(SonarConfigurationService.class);

		viewList.add(new PluginViewImpl(PluginViewEnum.DEFAULT, getDefaultAccess()));
		viewList.add(new PluginViewImpl(PluginViewEnum.ADMINISTRATION, sonarConfiguration.getAdminAccess()));
		return viewList;
	}
	...
	public PluginServiceMetadata getMetadata() throws PluginServiceException
	{
		...
		if (getServiceUUID() != null)
		{
			pluginServiceMetadata = new PluginMetadataImpl(getServiceUUID(), getDescription(), getType(),
					Category.QUALITY.getId(), new SonarQueues(), getPluginViews(), getVersion());
		}
	...
	public Set<String> findRoles()
	{
		...
		for (final SonarRole roles : SonarRole.values())
		{
