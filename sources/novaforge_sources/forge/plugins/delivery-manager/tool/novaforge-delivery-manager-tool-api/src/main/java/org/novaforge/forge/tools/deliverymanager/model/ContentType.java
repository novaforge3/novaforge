/*
 * Copyright (c) 2011-2015, BULL SAS, NovaForge Version 3 and above.
 *
 * This file is free software: you may redistribute and/or modify it under
 * the terms of the GNU Affero General Public License as published by the
 * Free Software Foundation, version 3 of the License.
 *
 * This file is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty
 * of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 * See the GNU Affero General Public License for more details.
 * You should have received a copy of the GNU Affero General Public License
 * along with this program. If not, see http://www.gnu.org/licenses.
 *
 * Additional permission under GNU AGPL version 3 (AGPL-3.0) section 7.
 *
 * If you modify this Program, or any covered work, by linking or combining
 * it with libraries listed in COPYRIGHT file at the top-level directory of
 * this distribution (or a modified version of that libraries), containing parts
 * covered by the terms of licenses cited in the COPYRIGHT file, the licensors
 * of this Program grant you additional permission to convey the resulting work.
 */
package org.novaforge.forge.tools.deliverymanager.model;

import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

/**
 * This enumeration describe the content available for a delivery
 * 
 * @author Guillaume Lamirand
 */
public enum ContentType
{
   /**
    * ECM Content
    */
   ECM
   {
      @Override
      public String getId()
      {
         return ECM_ID;
      }
   },
   /**
    * SCM Content
    */
   SCM
   {
      @Override
      public String getId()
      {
         return SCM_ID;
      }
   },
   /**
    * Bugtracker Content
    */
   BUG
   {
      @Override
      public String getId()
      {
         return BUG_ID;
      }
   },
   /**
    * File management Content
    */
   FILE
   {
      @Override
      public String getId()
      {
         return FILE_ID;
      }
   },
   /**
    * Delivery note Content
    */
   NOTE
   {
      @Override
      public String getId()
      {
         return NODE_ID;
      }
   };

   private static final String ECM_ID  = "ecm";
   private static final String SCM_ID  = "scm";
   private static final String BUG_ID  = "bug";
   private static final String FILE_ID = "file";
   private static final String NODE_ID = "note";

   public static ContentType getById(final String pId)
   {
      ContentType returnType = null;
      final ContentType[] values = ContentType.values();
      for (final ContentType type : values)
      {
         if (type.getId().equals(pId))
         {
            returnType = type;
         }
      }
      return returnType;
   }

   public abstract String getId();

   public static List<ContentType> list()
   {
      final List<ContentType> typeList = new ArrayList<ContentType>();
      final ContentType[] values = ContentType.values();
      Collections.addAll(typeList, values);
      return typeList;
   }
}
