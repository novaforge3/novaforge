 ...
 */
package org.novaforge.forge.tools.requirements.common.internal.handlers;
...
import org.novaforge.forge.tools.requirements.common.model.Permission;
...
public class RoleHandler
{
  ...
  public void starting()
  {
    try
    {
      ...
      if (!lock.exists())
      {
        log.info("initialization of requirements manager roles...");
        createOrUpdateRoles();
        lock.createNewFile();
        log.info("Requirements Manager Initialization : FINISHED SUCCESSFULLY.");
      }
      else
      {
        log.info("initialization of requirements manager roles has already been done. If you want to rebuild it, remove the lock file.");
      }
    }
    catch (Exception e)
    {
      log.error("Unable to initialize requirements manager roles", e);
    }
  ...
  {
    log.info("Roles for tool-requirements-manager are : " + pRoles);
    roles = pRoles;
  }
