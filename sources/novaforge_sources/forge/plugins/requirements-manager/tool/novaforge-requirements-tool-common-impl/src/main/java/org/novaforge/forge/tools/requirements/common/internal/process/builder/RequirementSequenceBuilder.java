...
import org.novaforge.forge.tools.requirements.common.internal.process.task.TestlinkSynchronizationTask;
 ...
 */
public class RequirementSequenceBuilder implements RequirementSequenceBuilderService
{
	...
	@Override
	public Sequence createNewSequence(final String pProjectId, final String pCodeRepositoryPath, final String pUserId, final List<String> pItems)
			throws Exception
	{
		if (pItems.size() > 0)
		{
			...
			{
				TestlinkSynchronizationTask testTask = new TestlinkSynchronizationTask("TEST TASK", pProjectId,
						pUserId, testRequirementConnector);
				activityTwo.addTasks(testTask);
			}
			if (pItems.contains("code"))
			{
				SVNSynchronizationTask SVNtask = new SVNSynchronizationTask("TEST SVN", pProjectId, pCodeRepositoryPath, pUserId,
						scmRequirementConnector);
				activityTwo.addTasks(SVNtask);
			}
