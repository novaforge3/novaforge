 ...
 * @date 25 juil. 2011
 */
public class TestlinkServiceImpl extends AbstractPluginService implements PluginService
{
	 ...
	 */
	public TestlinkServiceImpl(final UuidDAO pUuidDAO)
	{
		super(pUuidDAO);
	}
	...
	protected List<PluginView> buildPluginViews()
	{
		...
		TestlinkConfigurationService testlinkConfiguration = getPluginConfigurationService(TestlinkConfigurationService.class);
		viewList.add(new PluginViewImpl(PluginViewEnum.ADMINISTRATION, testlinkConfiguration.getAdminAccess()));
		return viewList;
	}
	...
	{
		return new TestlinkAssociation();
	}
	...
	public PluginServiceMetadata getMetadata() throws PluginServiceException
	{
		...
		if (getServiceUUID() != null)
		{
			pluginServiceMetadata = new PluginMetadataImpl(getServiceUUID(), getDescription(), getType(),
																										 Category.TESTMANAGEMENT.getId(), new TestlinkQueues(),
	...
	public Set<String> findRoles() throws PluginServiceException
	{
		...
		for (final TestlinkRolePrivilege roles : TestlinkRolePrivilege.values())
		{
