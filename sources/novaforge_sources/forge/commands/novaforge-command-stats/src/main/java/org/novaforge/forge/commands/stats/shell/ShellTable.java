...
public class ShellTable
{
   ...
   */
  private static int               maxColSize = 35;
  ...
  public void print()
  {
    ...
    final int[] sizes = new int[header.size()];
    updateSizes(sizes, header);
    for (final List<String> row : content)
    {
      updateSizes(sizes, row);
    }
    // Draw table
    final String headerLine = getRow(sizes, header, " | ");
  ...
  {
    int c = 0;
    for (final String cellContent : pRow)
    {
