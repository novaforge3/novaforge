...
import org.novaforge.forge.plugins.categories.beans.BugBeanImpl;
 ...
 */
public class BugTrackerDefinitionServiceImpl extends AbstractDefinitionService implements
    CategoryDefinitionService
{
  /**
   * The name of the property file used
   */
  private static final String                PROPERTY_FILE         = "categories.bugtracker.definition";
  /**
  ...
  private void buildAssociations()
  {
    ...
    // Build actions associations
    for (final BugTrackerAction notification : BugTrackerAction.values())
    {
      final Association assoc = new AssociationImpl(AssociationType.ACTION, notification.name(), getCategory());
      associations.add(assoc);
    }

    // Build replies associations
    for (final BugTrackerReply reply : BugTrackerReply.values())
    {
  ...
  private void setParameters()
  {
    ...
    for (final BugTrackerAction action : BugTrackerAction.values())
    {
      switch (action)
      {
        case createBug:
          actionParameter.put(action.name(), BugBeanImpl.class);
          break;
  ...
  {
    return BugTrackerCategoryService.class;
  }
