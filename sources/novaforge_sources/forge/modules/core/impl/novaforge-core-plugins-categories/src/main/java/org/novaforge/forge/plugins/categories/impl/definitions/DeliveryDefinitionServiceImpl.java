...
import org.novaforge.forge.core.plugins.categories.delivery.DeliveryCategoryService;
 ...
 */
public class DeliveryDefinitionServiceImpl extends AbstractDefinitionService implements
    CategoryDefinitionService
{
  /**
   * The name of the property file used
   */
  private static final String                PROPERTY_FILE         = "categories.delivery.definition";
  /**
  ...
  private void buildAssociations()
  {
    ...
    for (final DeliveryRequest request : DeliveryRequest.values())
    {
  ...
  {
    return DeliveryCategoryService.class;
  }
  ...
  public List<Association> getCompatibleAssociations(final Association pAssociation)
  {
    ...
    if ((getCategory().equals(pAssociation.getCategory())) && (AssociationType.REQUEST.equals(pAssociation.getType())))
    {
      ...
      if (DeliveryRequest.findBugsByVersion.name().equals(pAssociation.getName()))
      {
        name = BugTrackerReply.findProjectIssuesByParameters.name();
        category = Category.BUGTRACKER;
      }
      else if (DeliveryRequest.getBugsVersions.name().equals(pAssociation.getName()))
      {
        name = BugTrackerReply.getAllProjectVersions.name();
        category = Category.BUGTRACKER;
      }
      else if (DeliveryRequest.getDocumentsContent.name().equals(pAssociation.getName()))
      {
        name = ECMReply.copyRepositoryContent.name();
        category = Category.ECM;
      }
      else if (DeliveryRequest.getDocumentsList.name().equals(pAssociation.getName()))
      {
        name = ECMReply.getRepositoryTree.name();
        category = Category.ECM;
      }
      else if (DeliveryRequest.getSourcesContent.name().equals(pAssociation.getName()))
      {
        name = SCMReply.copySources.name();
        category = Category.SCM;
      }
      else if (DeliveryRequest.getSourcesList.name().equals(pAssociation.getName()))
      {
        name = SCMReply.getSourcesTree.name();
        category = Category.SCM;
      }
