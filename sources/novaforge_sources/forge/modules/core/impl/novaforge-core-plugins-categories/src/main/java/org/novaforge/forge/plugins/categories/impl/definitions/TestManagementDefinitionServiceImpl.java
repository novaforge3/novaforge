 ...
 */
public class TestManagementDefinitionServiceImpl extends AbstractDefinitionService implements
    CategoryDefinitionService
{
   ...
   */
  private static final String                PROPERTY_FILE         = "categories.testmanagement.definition";
  /**
  ...
  private void buildAssociations()
  {
    ...
    // Build notifications associations
    for (final TestManagementNotification notification : TestManagementNotification.values())
    {
      final Association assoc = new AssociationImpl(AssociationType.NOTIFICATION, notification.name(), getCategory());
      associations.add(assoc);
    }

    // Build replies associations
    for (final TestManagementReply reply : TestManagementReply.values())
    {
    ...
    // Build receive associations
    for (final TestManagementReceive receive : TestManagementReceive.values())
    {
      final Association assoc = new AssociationImpl(AssociationType.RECEIVE, receive.name(), getCategory());
      associations.add(assoc);
    }
  }
  ...
  private void setParameters()
  {
    ...
    for (final TestManagementNotification notification : TestManagementNotification.values())
    {
      ...
      {
        case testFailed:
          notificationParameter.put(notification.toString(), TestBeanImpl.class);
          break;
        default:
          break;
      }
  ...
  {
    return Category.TESTMANAGEMENT;
  }
  ...
  {
    return TestManagementCategoryService.class;
  }
  ...
  public List<Association> getCompatibleAssociations(final Association pAssociation)
  {
    ...
    if ((getCategory().equals(pAssociation.getCategory()))
        && (AssociationType.RECEIVE.equals(pAssociation.getType())))
    {
      if (TestManagementReceive.synchronizeRequirements.name().equals(pAssociation.getName()))
      {
        assoc.add(new AssociationImpl(AssociationType.SEND, RequirementsSend.updateRequirements.name(),
            Category.REQUIREMENTMANAGEMENT));
      }
    }
    return assoc;
  }
