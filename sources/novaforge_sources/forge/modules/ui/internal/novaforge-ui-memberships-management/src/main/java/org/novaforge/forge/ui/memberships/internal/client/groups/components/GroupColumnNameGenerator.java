 ...
 */
public class GroupColumnNameGenerator implements ColumnGenerator
{
  /**
   * Serial version id
   */
  ...
  public Object generateCell(final com.vaadin.ui.Table pSource, final Object pItemId, final Object pColumnId)
  {
    ...
    final String groupName = (String) item.getItemProperty(GroupItemProperty.NAME.getPropertyId()).getValue();
    final StringBuilder nameBuilder = new StringBuilder(groupName);
    final boolean editable = (Boolean) item.getItemProperty(GroupItemProperty.EDITABLE.getPropertyId())
        .getValue();

    if (!editable)
    {
      nameBuilder.append(" ").append(
          MembershipsModule.getPortalMessages().getMessage(pSource.getLocale(),
              Messages.MEMBERSHIPS_GROUPS_PUBLIC_INHERIT));
    }
