package org.novaforge.forge.ui.novadeploy.internal.client.rest;
...
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.AccountOrganizationResponse;
...
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.DeployResponse;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.DescriptorResponse;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.DescriptorsArborescenceResponse;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.DescriptorsResponse;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.EnvironmentResponse;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.EnvironmentsResponse;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.LogsResponse;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.Membership;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.MembershipResponse;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.NovadeployResourceBuilder;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.NovadeployUser;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.OrgResources;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.OrgResourcesResponse;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.ServiceResponse;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.SessionResponse;
import org.novaforge.forge.ui.novadeploy.internal.client.rest.data.StatusResponse;
...
//	private void handleCasAuthentication(NovadeployClient connector, ClientResponse clientResponse){
//		if(clientResponse.getStatus() == 302){
//			String casLocation = clientResponse.getHeaders().get("Location").get(0);
//			
//			
//			// Contact cas server
//			
//			WebResource resource = connector.getClient().resource(casLocation);
//
//			MultivaluedMap<String, String> map = new MultivaluedMapImpl();
//
//
//			ClientResponse casResponse = resource.get(ClientResponse.class);
//			
//		}
//	}
	// ///////////////////////////////////////////
			...
			final String cloudname, final String password, final String provider) throws NovadeployClientException
	{
		...
		try
		{
			...
			/*
			 * try { jo.getAsJsonArray("vdc");
			 * jo.getAsJsonArray("network"); } catch (Exception e) {
			 * JsonArray vdc = new JsonArray();
			 * vdc.add(jo.get("vdc")); JsonArray network = new
			 * JsonArray(); network.add(jo.get("network"));
			 * jo.add("vdc", vdc); jo.add("network", network); }
			 */
			...
			throws NovadeployClientException
	{
		...
		if (response.hasError())
		{
			throw new NovadeployClientException(String.format("Error in NovaDeploy response", project), response);
		}
	}
	...
	public void createUser(NovadeployClient connector, final NovadeployUser pUser) throws NovadeployClientException
	{
		...
		if (response.getError() != null && response.getErrorNumber() != null && !response.getErrorNumber().equals("1"))
		{
	...
	public void createAccount(final NovadeployClient pClient, String pProjectId) throws NovadeployClientException
	{
		...
		{
			throw new NovadeployClientException(String.format("Failed to create project [project=%s].", pProjectId),
					response);
		}
	}
	...
	public void lockAccount(NovadeployClient connector, String pProjectId) throws NovadeployClientException
	{
		...
		{
			throw new NovadeployClientException(
					String.format("Error detected on Novadeploy [project=%s].", pProjectId), response);
		}
	}
	...
	public void unlockAccount(NovadeployClient connector, String pProject) throws NovadeployClientException
	{
		...
		if (response.hasError())
		{
			throw new NovadeployClientException(String.format("Failed to unlock project on Novadeploy [project=%s].",
					pProject), response);
		}
	}
			...
			throws NovadeployClientException
	{
		...
		try
		{
			...
			/*
			 * try { jo.getAsJsonArray("memberships"); } catch
			 * (Exception e) { JsonArray membershipsJson = new
			 * JsonArray();
	 ...
	 */
	@Override
	public Membership getUserMembershipFromProject(NovadeployClient connector, String pProject)
			throws NovadeployClientException
	{
		...
		try
		{
			WebResource resource = connector.getClient().resource(
					String.format("%s%s", connector.getToolUrl(),
							NovadeployRestConstant.Novadeploy_METHOD_GET_USER_ROLE_FROM_PROJECT));
			...
			response = gson.fromJson(jo, MembershipResponse.class);
	 ...
	 */
	@Override
	public void createDescriptor(NovadeployClient connector, String descriptorName, String descriptorContent,
			String project)
	...
	throws NovadeployClientException
	{
		...
		{
			throw new NovadeployClientException(String.format(
					"Error detected on NovaDeploy : unable to add descriptor on project  [project=%s]. %s", project,
					response.parserMessageError), response);
		}
	...
	public void forkDescriptor(NovadeployClient connector, String project, String descriptorName,
			int descriptorVersion, String forkName) throws NovadeployClientException
	{
		ServiceResponse response;
		try
		{
		...
		{
			throw new NovadeployClientException(String.format(
					"Error detected on NovaDeploy : unable to fork descriptor on project  [project=%s].", project),
					response);
		}
	}
	 ...
	 */
	@Override
	public void updateDescriptor(NovadeployClient connector, String descriptorName, String descriptorContent,
			String project)
	 ...
	 */
	@Override
	public DescriptorResponse getDescriptor(NovadeployClient connector, int descriptorVersion, String descriptorName,
			String project)
	 ...
	 */
	@Override
	public long deployEnvironment(NovadeployClient connector, String descriptorName, int descriptorVersion,
			String project) throws NovadeployClientException
	{
	...
	public String[] deploymentStatus(NovadeployClient connector, long timeRefId) throws NovadeployClientException
	{
		...
		try
		{
			...
			jo = jo.get("response").getAsJsonObject();
			/*
			 * try { jo.getAsJsonArray("logs"); } catch (Exception e)
			 * { JsonArray logsJson = new JsonArray();
			 * logsJson.add(jo.get("logs")); jo.add("logs", logsJson);
			 * }
			 */
			...
			throws NovadeployClientException
	{
		...
		try
		{
			WebResource resource = connector.getClient().resource(
					String.format("%s%s%s", connector.getToolUrl(),
							NovadeployRestConstant.Novadeploy_METHOD_GET_ENVIRONMENT, "/" + Long.toString(timeRefId)));
	...
	@Override
	public OrgResources getAccountResources(NovadeployClient connector, String project)
			throws NovadeployClientException
	{
		OrgResourcesResponse response = null;

		try
		{
			WebResource resource = connector.getClient().resource(
					String.format("%s%s", connector.getToolUrl(),
							NovadeployRestConstant.Novadeploy_METHOD_RESOURCES_CLOUD));
			...
			response = gson.fromJson(jo, OrgResourcesResponse.class);

		} catch (Exception e)
		{
			throw new NovadeployClientException(String.format(
					"Failed to get cloud account resources with project [project=%s].", project), e);
		}
		if (response.hasError())
		{
			throw new NovadeployClientException(String.format("An error occured [error=%s].", response.getError()),
		...
		return response.getOrgResources();
	}
