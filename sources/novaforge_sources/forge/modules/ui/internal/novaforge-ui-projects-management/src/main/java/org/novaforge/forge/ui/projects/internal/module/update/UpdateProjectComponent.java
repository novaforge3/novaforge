 ...
 */
package org.novaforge.forge.ui.projects.internal.module.update;
 ...
 */
public class UpdateProjectComponent extends AbstractPortalComponent
{
   ...
   */
  public UpdateProjectComponent(final PortalContext pPortalContext)
  {
    super(pPortalContext);
    projectId = pPortalContext.getAttributes().get(PortalContext.KEY.PROJECTID);

    final ProjectView view = new ProjectViewImpl(false);
    view.setStyleName(NovaForge.PROJECT_APPLICATION_CONTENT);
    presenter = new ProjectPresenter(view, pPortalContext)
    {
      ...
      {
        return UpdateProjectModule.getPortalModuleId();
      }
    };
    setContent(view);
  }
  ...
  public void init()
  {
    ...
    try
    {
      final Project project = ProjectServices.getProjectPresenter().getProject(projectId, true);
      presenter.refresh(project);
    }
    catch (final ProjectServiceException e)
    {
      ExceptionCodeHandler.showNotificationError(ProjectServices.getPortalMessages(), e, UI.getCurrent()
          .getLocale());
    }
  }
  ...
  {
    // final Notification notification = new Notification(ProjectServices.getPortalMessages().getMessage(
    // UI.getCurrent().getLocale(), Messages.PROJECT_UPDATE_SUCCESS), Type.HUMANIZED_MESSAGE);
    final TrayNotification notification = new TrayNotification(ProjectServices.getPortalMessages()
        .getMessage(UI.getCurrent().getLocale(), Messages.PROJECT_UPDATE_SUCCESS),
