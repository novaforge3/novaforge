...
import org.novaforge.forge.ui.projects.internal.client.admin.components.ColumnRequestActionsGenerator;
 ...
 */
public class ProjectsRequestPresenter extends AbstractAdminPresenter implements Serializable
{
  /**
   ...
   * Logger component
   */
  private static final Log          LOGGER           = LogFactory.getLog(ProjectsRequestPresenter.class);
  /**
   * Content of project view
   */
  private final ProjectsRequestView view;
   ...
   *          project id
   */
  public ProjectsRequestPresenter(final ProjectsRequestView pView, final PortalContext pPortalContext)
  {
  ...
  private void initProjectsList()
  {
    ...
    view.getProjectsTable().addGeneratedColumn(ProjectItemProperty.ACTIONS.getPropertyId(),
        new ColumnRequestActionsGenerator(this));
    ...
    view.getProjectsTable().setColumnExpandRatio(ProjectItemProperty.DESCRIPTION.getPropertyId(), 0.3f);
    view.getProjectsTable().setColumnExpandRatio(ProjectItemProperty.AUTHOR.getPropertyId(), 0.2f);
  ...
  public void addListeners()
  {
    ...
    view.getInValidProjectCancelButton().addClickListener(new ClickListener()
    {
      ...
      public void buttonClick(final ClickEvent event)
      {
        view.getProjectsTable().unselect(projectItemId);
        projectItemId = null;
        UI.getCurrent().removeWindow(view.getInValidProjectWindow());
      }
    });
    view.getInValidProjectConfirmButton().addClickListener(new Button.ClickListener()
    {
      ...
      public void buttonClick(final ClickEvent event)
      {
        if (projectItemId != null)
        {
          try
          {
            ProjectServices.getProjectPresenter().rejectProject(projectItemId,
                view.getInValidProjectText().getValue());
            UI.getCurrent().removeWindow(view.getInValidProjectWindow());
            refresh();
          }
  ...
  /**
   * Called on validate button in actions column
   *
   * @param pItemId
   *          project id selected
   */
  ...
  /**
   * Called on invalidate button in actions column
   *
   * @param pItemId
   *          project id selected
   */
  ...
  {
    projectItemId = (String) pItemId;
    view.getInValidProjectText().setValue("");
    UI.getCurrent().addWindow(view.getInValidProjectWindow());
  ...
  protected void refreshContent()
  {
    if (projectItemId != null)
    {
      if ((view.getProjectsTable().containsId(projectItemId)) && (view.getProjectsTable().isSelected(projectItemId)))
      {
        view.getProjectsTable().unselect(projectItemId);
      }
      projectItemId = null;
