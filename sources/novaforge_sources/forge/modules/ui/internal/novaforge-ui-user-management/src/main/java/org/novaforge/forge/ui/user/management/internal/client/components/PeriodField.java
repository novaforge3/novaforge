...
public class PeriodField extends HorizontalLayout
{
  ...
  /**
   * Initialize the allowed range
   * By defualt, all are allowed
   */
  ...
  private void fillRangeComboBox()
  {
    ...
    for (final PeriodRange range : ranges)
    {
      ...
      valueComboBox.setItemCaption(range.getId(),
          AdminModule.getPortalMessages().getMessage(UI.getCurrent().getLocale(), range.getI18NMessageId()));
    }
